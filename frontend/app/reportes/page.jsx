...
export default function Compras() {
  ...
  const descargarJSONReporte = (dataR)=>{
    ...
    const scriptURL = "http://44.212.165.114:3001/api/v1/reportes/descargarJSON";
  ...
  const handleUpload = (event) => {
    ...
    axios.post('http://44.212.165.114:3001/api/v1/reportes/cargarPDF', formData)
      .then((response) => {
        setTimeout(()=>{data();},500)
      })
      .catch((error) => {
  ...
  const columns = [
    ...
    // },
    {
      ...
      sortable: false,
      flex: 2.7,
      renderCell: (params) => params.row.nombre_archivo_status?<a href={"http://44.212.165.114:3001/pdfs/"+params.row.nombre_archivo_status} target='_blank'>{params.row.nombre_archivo_status}</a>:<a onClick={()=>{setReporteID(params.row.reporte_id);document.querySelector("#file").click()}}>Subir archivo de aceptación</a>
    },
    // {
